...
public class ProjectileAbility extends Ability {
    ...
    public void tick(LivingEntity entity, AbilityEntry entry, IPowerHolder holder, boolean enabled) {
        if (!entity.level.isClientSide && enabled) {
            ...
            EntityType.loadEntityRecursive(compound, world, (projectile) -> {
                ...
                ((Projectile) projectile).shoot(f, f1, f2, velocity, inaccuracy);
                ...
                if (projectile instanceof ThrowableProjectile) {
                    ((ThrowableProjectile) projectile).setOwner(entity);
                }
