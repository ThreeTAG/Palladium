...
public class SwingAnchorRenderer extends EntityRenderer<SwingAnchor, SwingAnchorRenderState> {
    ...
    @Override
    public void render(SwingAnchorRenderState renderState, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight) {
        super.render(renderState, poseStack, bufferSource, packedLight);

        if (renderState.owner instanceof AbstractClientPlayer player) {
            poseStack.pushPose();
            var ownerPos = reusedState.owner.getPosition(renderState.partialTick);
            ...
            if (beamRenderer != null) {
                ...
                if (renderState.despawning) {
                    for (Beam beam : beamRenderer.beams()) {
                        ...
                        beam.render(DataContext.forEntity(player), targetPos,
                                anchorPos.subtract(ownerPos), sizeMultiplier, 1F, renderState.opacity, poseStack, bufferSource,
                                packedLight, player.tickCount, renderState.partialTick);
                    }
                } else {
                    beamRenderer.renderOnPlayer(player, ownerPos,
                            anchorPos, 1F, renderState.opacity, poseStack, bufferSource,
                            packedLight, renderState.partialTick);
                }

                beamRenderer.render(DataContext.forEntity(player), anchorPos.subtract(ownerPos),
                        anchorPos.subtract(ownerPos).add(0, 100, 0), sizeMultiplier, 1F, renderState.opacity, poseStack, bufferSource,
                        packedLight, player.tickCount, renderState.partialTick);
            }
