...
public class TexturedBeamRenderer extends BeamRenderer {
    ...
    public TexturedBeamRenderer(TextureReference texture, Vector2f size, float rotation, float rotationSpeed) {
    ...
    public void render(DataContext context, Vec3 origin, Vec3 target, Vec2 sizeMultiplier, float lengthMultiplier, float opacityMultiplier, PoseStack poseStack, MultiBufferSource bufferSource, int packedLightIn, int ageInTicks, float partialTick) {
        ...
        var vertex = bufferSource.getBuffer(RenderType.beaconBeam(texture, false));
        ...
        for (int i = 0; i < segments; i++) {
            ...
            RenderUtil.renderTexturedBox(poseStack, vertex, box, RenderUtil.FULL_WHITE, packedLightIn, heightMultiplier);
        }
