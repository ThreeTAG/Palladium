    ...
    public void render(DataContext context, PoseStack poseStack, MultiBufferSource bufferSource, EntityModel<LivingEntityRenderState> parentModel, LivingEntityRenderState state, State layerState, int packedLight, float partialTick, float xRot, float yRot) {
        ...
        if (entity != null) {
            for (Spark spark : layerState.sparks) {
                ...
                this.laserRenderer.faceAndRender(poseStack, bufferSource, spark.pos1, spark.pos2, spark.ticks, partialTick, 1F, opacity);
                this.laserRenderer.faceAndRender(poseStack, bufferSource, spark.pos1, spark.pos3, spark.ticks, partialTick, 1F, opacity);
                poseStack.popPose();
            }
        }
    }
