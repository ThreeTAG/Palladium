...
public class DataAttachmentCommand {
    ...
    public static void register() {
        PalladiumCommand.COMMAND_CALLBACK.register((cmd, context) -> {
            cmd.then(Commands.literal("data-attachment").requires((player) -> {
    ...
    public static int get(CommandSourceStack source, Entity entity, ResourceLocation typeId) throws CommandSyntaxException {
        PlatformAttachmentType<?> type = DataAttachmentLoader.INSTANCE.get(typeId);

        if (type == null) {
            throw ERROR_UNKNOWN_TYPE.create(typeId.toString());
        }

        Object value = type.get(entity);

        if (value == null) {
            source.sendFailure(Component.translatableEscape(TRANS_NO_DATA, typeId));
            return 0;
        }
