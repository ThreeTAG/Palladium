...
public abstract class ValueSerializer<T extends Value> implements Documented<Value, T> {
    ...
    private static final BiMap<ResourceLocation, ValueSerializer<?>> TYPES = HashBiMap.create();

    public static final Codec<ValueSerializer<?>> TYPE_CODEC = ResourceLocation.CODEC.flatXmap(resourceLocation -> {
        ValueSerializer<?> serializer = TYPES.get(resourceLocation);
    ...
    public static <T extends Value> ValueSerializer<T> register(ResourceLocation id, ValueSerializer<T> serializer) {
    ...
    public static Map<ResourceLocation, ValueSerializer<?>> getTypes() {
