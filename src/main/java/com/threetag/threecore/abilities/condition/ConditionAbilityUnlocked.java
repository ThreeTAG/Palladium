...
public class ConditionAbilityUnlocked extends Condition
{

    public static final ThreeData<String> ABILITY_ID = new ThreeDataString("ability_id").setSyncType(EnumSync.SELF).enableSetting("ability_id", "The id for the ability that must be unlocked for the condition to be true.");

    public ConditionAbilityUnlocked(Ability ability)
    {
    ...
    @Override public boolean test(EntityLivingBase entity)
    {
        ...
        this.dataManager.set(Condition.NAME, dependentAbility == null ? new TextComponentString("") : new TextComponentTranslation("ability.condition.threecore.ability_unlocked", dependentAbility.getDataManager().get(Ability.TITLE)));
