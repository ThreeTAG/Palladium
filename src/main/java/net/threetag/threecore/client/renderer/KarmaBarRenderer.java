    ...
    public void renderHUD(GuiScreenEvent.DrawScreenEvent e) {
        if (e.getGui() instanceof InventoryScreen || e.getGui().getClass().toString().equals("class top.theillusivec4.curios.client.gui.CuriosScreen")) {
            ...
            mc.player.getCapability(CapabilityKarma.KARMA).ifPresent((k) -> {
                ...
                mc.ingameGUI.blit(e.getMatrixStack(), mainWindow.getScaledWidth() / 2 - 91, (mainWindow.getScaledHeight() / 2) - (((ContainerScreen<?>) e.getGui()).getYSize() / 2) - 10, 0, 0, 182, 5);
                mc.ingameGUI.blit(e.getMatrixStack(), mainWindow.getScaledWidth() / 2 - 85 + (int) (f * 170) - 3, (mainWindow.getScaledHeight() / 2) - (((ContainerScreen<?>) e.getGui()).getYSize() / 2) - 12, 0, 5, 6, 9);
                for (KarmaClass classes : KarmaClass.VALUES) {
                    if (classes != KarmaClass.NEUTRAL) {
                        ...
                        mc.ingameGUI.blit(e.getMatrixStack(), mainWindow.getScaledWidth() / 2 - 85 + (int) (f1 * 170) - 1, (mainWindow.getScaledHeight() / 2) - (((ContainerScreen<?>) e.getGui()).getYSize() / 2) - 11, 6, 5, 2, 7);
                    }
                }
