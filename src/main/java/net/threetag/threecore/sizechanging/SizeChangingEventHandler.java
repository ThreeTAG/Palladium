...
public class SizeChangingEventHandler
{
	...
	public static void onLivingTick(LivingEvent.LivingUpdateEvent e)
	{
		e.getEntity().getCapability(CapabilitySizeChanging.SIZE_CHANGING).ifPresent(size -> {
			...
			{
				if (!e.getEntityLiving().func_233570_aj_() && (e.getEntityLiving().getHeldItemMainhand().getItem() == Items.PAPER
	...
	public static void onRegisterAttributes(RegistryEvent.Register<Attribute> event){
		event.getRegistry().register(TCAttributes.SIZE_HEIGHT);
		event.getRegistry().register(TCAttributes.SIZE_WIDTH);
	}
