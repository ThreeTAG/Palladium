        ...
        if (CapabilityFluidHandler.FLUID_HANDLER_ITEM_CAPABILITY != null)
            stack.getCapability(CapabilityFluidHandler.FLUID_HANDLER_ITEM_CAPABILITY).ifPresent(fluidHandler -> {
                ...
                if (!fluidStack.isEmpty()) {
                    tooltip.add(fluidStack.getDisplayName().deepCopy().mergeStyle(TextFormatting.GRAY));
                    tooltip.add(new StringTextComponent(TCFluidUtil.getFormattedFluidInfo(fluidStack.getAmount(), fluidHandler.getTankCapacity(0))).mergeStyle(TextFormatting.GRAY));
                }
            });
    }
