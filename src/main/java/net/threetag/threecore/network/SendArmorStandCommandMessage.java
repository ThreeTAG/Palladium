    ...
    public void handle(Supplier<NetworkEvent.Context> ctx) {
        ctx.get().enqueueWork(() -> {
            if(this.argument.equalsIgnoreCase("reload")) {
                ...
                if (pose == null)
                    Minecraft.getInstance().player.sendMessage(new TranslationTextComponent("commands.armorstandpose.pose_not_found", argument).func_240701_a_(TextFormatting.RED), null);
                else {
                    ...
                    if (res instanceof EntityRayTraceResult && ((EntityRayTraceResult) res).getEntity() instanceof ArmorStandEntity) {
                        ...
                        Minecraft.getInstance().player.sendMessage(new TranslationTextComponent("commands.armorstandpose.no_armor_stand").func_240701_a_(TextFormatting.RED), null);
                    }
                }
            }
        });
