...
public class ListPowerTab extends PowerTab {
    ...
    public void onOpened() {
        ...
        if (this.list != null)
            this.screen.addRenderableWidget(this.list);
    }
    ...
    public static class ListEntry extends AbstractSelectionList.Entry<ListEntry> {
        ...
        public void renderContent(GuiGraphics guiGraphics, int mouseX, int mouseY, boolean hovering, float partialTick) {
            guiGraphics.blit(RenderPipelines.GUI_TEXTURED, PowersScreen.WIDGETS, this.getX(), this.getY(), 0, 130 + (this.abilityInstance.isUnlocked() ? (hovering ? 2 : 0) : 1) * 26, this.getWidth(), this.getHeight(), 256, 256);

            if (this.abilityInstance.isUnlocked()) {
                IconRenderer.drawIcon(this.abilityInstance.getAbility().getProperties().getIcon(), this.minecraft, guiGraphics, DataContext.forAbility(this.minecraft.player, this.abilityInstance), this.getX() + 5, this.getY() + 5);
            } else {
                guiGraphics.blit(RenderPipelines.GUI_TEXTURED, PowersScreen.WIDGETS, this.getX() + 5, this.getY() + 5, 90, 83, 16, 16, 256, 256);
...
//                }
            ...
            guiGraphics.drawString(this.minecraft.font, Language.getInstance().getVisualOrder(this.minecraft.font.substrByWidth(abilityInstance.getAbility().getDisplayName(), 180)), this.getX() + 30, this.getY() + 9, this.abilityInstance.isUnlocked() ? (hovering ? 0xfcfc7e : 0x675d49) : 0x332e25, false);
