...
public class TreePowerTab extends PowerTab {
    ...
    public void mouseClicked(MouseButtonEvent event, boolean isDoubleClick) {
        ...
        TreeAbilityWidget entry = this.getAbilityHoveredOver((int) (event.x() - i - 9), (int) (event.y() - j - 18), i, j);

        if (entry != null && entry.parentsUnlocked) {
            entry.abilityInstance.getAbility().getStateManager().getUnlockingHandler().onClicked(this.minecraft.player, entry.abilityInstance);
        }
    }
