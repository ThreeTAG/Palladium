...
public record ParticleEmitter(@Nullable BodyPart anchor, float amount, PerspectiveValue<Vector3f> offset,
    ...
    public static final Codec<ParticleEmitter> CODEC = RecordCodecBuilder.create(instance -> instance.group(
            ...
            PerspectiveValue.codec(PalladiumCodecs.VOXEL_VECTOR_3F).optionalFieldOf("motion", new PerspectiveValue<>(new Vector3f())).forGetter(ParticleEmitter::motion),
            PerspectiveValue.codec(PalladiumCodecs.VOXEL_VECTOR_3F).optionalFieldOf("motion_random", new PerspectiveValue<>(new Vector3f())).forGetter(ParticleEmitter::motionRandom),
            PerspectiveAwareConditions.CODEC.optionalFieldOf("conditions", PerspectiveAwareConditions.EMPTY).forGetter(ParticleEmitter::conditions)
    ...
    private void spawnParticleOnPlayer(Level level, AbstractClientPlayer player, ParticleOptions particleOptions, RandomSource random, float partialTick) {
        ...
        Vec3 pos = this.anchor != null ? ModelUtil.getInWorldPosition(this.anchor, offset, player, partialTick) :
        ...
        if (this.anchor != null) {
            ...
            var transformed = ModelUtil.getInWorldPosition(this.anchor, combined, player, partialTick);
