...
public class AnimationAbility extends Ability {
    ...
    public static final MapCodec<AnimationAbility> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group(
            PalladiumCodecs.listOrPrimitive(Identifier.CODEC).fieldOf("animations").forGetter(a -> a.animations),
            propertiesCodec(), stateCodec(), energyBarUsagesCodec()
    ).apply(instance, AnimationAbility::new));

    public final List<Identifier> animations;

    public AnimationAbility(List<Identifier> animations, AbilityProperties properties, AbilityStateManager stateManager, List<EnergyBarUsage> energyBarUsages) {
    ...
    public static class Serializer extends AbilitySerializer<AnimationAbility> {
        ...
        public void addDocumentation(CodecDocumentationBuilder<Ability, AnimationAbility> builder, HolderLookup.Provider provider) {
            builder.setName("Animation")
                    ...
                    .addExampleObject(new AnimationAbility(Collections.singletonList(Identifier.parse("test:animation_id")), AbilityProperties.BASIC, AbilityStateManager.EMPTY, List.of()));
        }
    }
}
