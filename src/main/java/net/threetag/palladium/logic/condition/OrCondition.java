...
public record OrCondition(List<Condition> conditions) implements Condition {
    ...
    public static class Serializer extends ConditionSerializer<OrCondition> {
        ...
        @Override
        public void addDocumentation(CodecDocumentationBuilder<Condition, OrCondition> builder, HolderLookup.Provider provider) {
            builder.setName("OR")
                    .setDescription("Allows you to group multiple conditions into one using the OR logic. At least one of the given conditions must be true for this one to be true aswell.")
                    .add("conditions", TYPE_CONDITION_LIST, "List of conditions")
                    .addExampleObject(new OrCondition(Arrays.asList(new CrouchingCondition(), new DayCondition())));
        }
    }
}
